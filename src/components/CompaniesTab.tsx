...
export const CompaniesTab: React.FC<CompaniesTabProps> = ({
  ...
  // Filter companies matching search
  ...
  return (
    <div className="space-y-4 animate-fade-in pb-12">
      ...
      {/* ------------------------------------------------------------- */}
      {/* COMPANIES LIST - CLEAN & STRUCTURED CARDS                     */}
      {/* ------------------------------------------------------------- */}
          ...
          {filtered
            ...
            .map((company) => {
              ...
              return (
                <div
                  key={company.id}
                  className="bg-white border border-slate-200 hover:border-slate-300 rounded-2xl p-4 sm:p-5 transition-all duration-200 shadow-xs flex flex-col justify-between relative overflow-hidden"
                >
                  ...
                  {/* Header: Logo, Name, Badge & Status - Unified Horizontal Layout */}
                  <div className="flex items-center justify-between gap-3 pt-1">
                    <div className="flex items-center gap-3.5 min-w-0 flex-1">
                      <div className="rounded-2xl border border-slate-200/80 p-1 bg-slate-50/80 shrink-0 shadow-xs">
                        <CompanyBrandLogo
                      ...
                      <div className="min-w-0 flex-1 space-y-1">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          {company.badge && (
                            <span
                              className="text-[10px] font-extrabold px-2 py-0.5 rounded-full border leading-tight shrink-0"
                              style={{
                        ...
                        <div className="flex items-center justify-between gap-2">
                          {isRegistered ? (
                            <span className="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-md">
                              ...
                              <span className="truncate">{t.registeredBadge}</span>
                            </span>
                          ) : isPending ? (
                            <span className="inline-flex items-center gap-1 text-[11px] font-bold text-amber-700 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-md">
                              ...
                              <span className="truncate">{t.pendingBadge}</span>
                            </span>
                          ) : (
                  ...
                  {/* Promo Code & Account Box */}
                  <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    {/* Promo Code Banner */}
                    <div
                      onClick={() => handleCopyPromo(company)}
                      className="p-3 rounded-xl border flex items-center justify-between cursor-pointer transition-all active:scale-98 shadow-2xs hover:shadow-xs"
                      style={{
                    ...
                    >
                      <div className="space-y-0.5">
                        ...
                        <span
                          className="font-mono font-black text-sm tracking-wider block"
                      ...
                      <div
                        className="w-8 h-8 rounded-lg flex items-center justify-center bg-white shadow-2xs border"
                    ...
                    {userAccount ? (
                      <div className="p-3 rounded-xl border border-slate-200 bg-slate-50 flex items-center justify-between shadow-2xs">
                        ...
                        {isRegistered && (
                          <button
                            ...
                            className="h-8 px-2.5 rounded-lg text-xs font-bold border flex items-center gap-1 shrink-0 active:scale-95 transition-transform cursor-pointer"
                            style={{
                              color: company.color,
                      ...
                      <div className="p-3 rounded-xl border border-slate-200 bg-slate-50/60 flex items-center justify-between text-xs text-slate-500">
                        <span className="text-xs font-medium">{t.notRegisteredBadge}</span>
                        <button
                          onClick={() => onOpenRegister(company)}
                          className="h-8 px-3 rounded-lg bg-white border border-slate-200 text-slate-800 font-bold text-xs hover:bg-slate-100 transition-colors shadow-2xs cursor-pointer"
                        >
                          {t.linkAccount}
                        </button>
                      </div>
                  ...
                  {/* Action Buttons Row */}
                  <div className="mt-4 pt-3.5 border-t border-slate-100 grid grid-cols-2 sm:grid-cols-4 gap-2">
                    <a
                      href={company.affiliate_link || '#'}
                      target="_blank"
                      rel="noreferrer"
                      className="h-10 px-3 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-1.5 active:scale-95 text-white shadow-xs cursor-pointer"
                      style={{ backgroundColor: company.color }}
                    >
                      <ExternalLink className="w-3.5 h-3.5 shrink-0" />
                      <span>{t.registerAndSite}</span>
                    </a>
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="h-10 px-3 rounded-xl text-xs font-bold border transition-all flex items-center justify-center gap-1.5 active:scale-95 bg-slate-50 hover:bg-slate-100 text-slate-800 border-slate-200 shadow-xs cursor-pointer"
                    >
                    ...
                    <button
                      onClick={() => onOpenRegister(company)}
                      className="h-10 px-3 rounded-xl text-xs font-bold bg-slate-100 hover:bg-slate-200 text-slate-800 border border-slate-200 transition-all active:scale-95 flex items-center justify-center gap-1.5 cursor-pointer"
                    >
                      <Edit3 className="w-3.5 h-3.5 text-slate-600" />
                      <span>{isRegistered ? t.editAccount : t.linkAccount}</span>
                    </button>

                    <button
                      onClick={() => onOpenDetails(company)}
                      className="h-10 px-3 rounded-xl bg-white hover:bg-slate-50 text-slate-600 hover:text-slate-800 text-xs font-bold border border-slate-200 transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
                    >
