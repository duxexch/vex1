        ...
        <div className="p-4 sm:p-5 overflow-y-auto flex-1">
          {success ? (
            <SuccessAnimation
              title={
                lang === 'ar'
                  ? 'تم استلام طلب التعويض'
                  : lang === 'es'
                  ? 'Solicitud de Reembolso Recibida'
                  : lang === 'ru'
                  ? 'Запрос на компенсацию принят'
              ...
              subtitle={
                lang === 'ar'
                  ? 'تم تسجيل قسيمة الرهان وتشفيرها في السيرفر لمنع التكرار، وسيتم إيداع الرصيد في محفظتك فور اكتمال التدقيق.'
                  : 'The bet slip has been verified and encrypted. Balance will be unlocked upon audit completion.'
              }
              details={[
                {
                  label: lang === 'ar' ? 'الشركة المعتمدة' : 'Company',
                  value: activeAccount?.company_name || selectedCompanyId,
                },
                {
                  label: t.currencyLabel,
                  value: `${activeCurrencyObj.flag} ${selectedCurrency} (${getCurrencyName(activeCurrencyObj, lang)})`,
                },
                {
                  label: t.betSlipLabel,
                  value: betSlipId,
                  isMono: true,
                },
                ...(screenshotName
                  ? [{ label: t.screenshotLabel, value: screenshotName }]
                  : []),
              ]}
              onDone={handleDoneManual}
              doneText={lang === 'ar' ? 'العودة للمحفظة' : 'Back to Wallet'}
            />
          ) : (
